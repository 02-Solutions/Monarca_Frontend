...
const Table: React.FC<TableProps> = ({ columns, data, itemsPerPage = 5 }) => {
  ...
  {currentItems.map((row) => (
    <React.Fragment key={row.id}>
      <tr className="bg-[#4C6997] text-white text-center">
        ...
        {/* Celda Acción: borde derecho sólido, sin redondeo */}
        <td className="px-4 py-3 border-r border-[#0a2c6d]">
        <button
        className="rounded border border-gray-300 px-3 py-1 text-black bg-white hover:bg-[#0a2c6d] hover:text-white focus:bg-[#0a2c6d] focus:text-white transition-colors"
  ...
  Ver detalle
      ...
      {expandedRowId === row.id && (
        <tr>
          <td colSpan={columns.length + 2} className="bg-white text-black p-4 rounded-b-lg">
